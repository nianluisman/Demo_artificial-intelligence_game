...
public class Enemy : AnimatedSprite
{
	...
	public override void _Ready()
	{
		...
		MainCharacter = GetParent().GetNode<Player>("Player");
	}
	...
	private void WalkPath(float delta)
	{
		...
		if(AstarPathFind.SetAstarPath(this.GlobalPosition, MainCharacter.Position))
				{
					// Set path target as the first node in the found path
					PlayerPathTarget = 1;
				}
		...
		if(PlayerPathTarget < AstarPathFind.PathNodeList.Count)
		{
			...
			if( MainCharacter.Position - targetNode < new Vector2(50,50)){
