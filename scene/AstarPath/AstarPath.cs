...
{
	[Export]
	public bool VisualizeAstarPath;                                 // If the path is drawn on in scene
	[Export]
	public bool UseDiagonalPathFinding = true;                      // If path finding uses diagonal path.
	private Godot.Collections.Array NonWalkableTiles;               // Array of non-walkable tiles
	public List<Vector2> PathNodeList = new List<Vector2>();        // List of path nodes
	...
	{
		var mapSize = TileMapWorld.GetUsedRect(); //Get all non empty tiles.  
		MapSize = new Vector2(mapSize.Size.x,mapSize.Size.y); //set map size.
	}
